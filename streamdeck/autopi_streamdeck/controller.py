...
def run(cfg: Config) -> int:
    ...
    def on_press(_deck, key, pressed):
        ...
        if not pages:
            return
        slot = deck_layout.slot_for_physical(key, deck.key_count(), cfg.rotation)
    ...
    try:
        while True:
            try:
                ...
                state["pages"] = pages
                _paint(deck, pages[state["page"] % len(pages)], catalog, deck_layout, cfg)
            except httpx.HTTPError as exc:
...
def _paint(deck, page, catalog, deck_layout, cfg: Config):
    size = deck.key_image_format()["size"]
    from StreamDeck.ImageHelpers import PILHelper
    for slot, action_id in enumerate(page):
        phys = deck_layout.rotated_index(slot, deck.key_count(), cfg.rotation)
        ...
        if action_id == deck_layout.PAGE_NEXT:
            ...
            label, color = "", "#111827"
        img = render_key(label, color, size, cfg.rotation)
        if img is not None:
