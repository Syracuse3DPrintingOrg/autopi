...
def test_create_can_command_action_raw_mode(client):
    ...
    # The dispatch registry refuses to run any driver that reports itself
    # unavailable (see actions/registry.py's run()), which is the case here
    # since there is no CAN hardware in CI; that gate is what the "can"
    # driver already relies on too. Calling the driver's execute() directly
    # (test_can_command.py) is what exercises the simulated-send fallback.
...
def test_create_and_run_can_command_action_with_database(client):
    ...
    # Same availability gate as the raw-mode test above (no CAN hardware in
    # CI); the action was still created correctly and points at the right
    # database/message, which test_can_command.py exercises against the
    # driver directly to confirm the encode and simulated send work.
